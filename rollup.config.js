...
const getPrefix = (p) =>
  p === "" || p === "gen/ts" || p === "gen/ts/openmarketplaceengine"
    ? p
    : getPrefix(path.dirname(p));

// full path will be something like:
// gen/ts/some-protos/idl/some/protos/greeter/v1beta1/greeter.ts
const getOutputDir = (fullPath) =>
  path.join("dist", fullPath.substring(getPrefix(fullPath).length));
